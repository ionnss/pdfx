...
impl Exporters {
    pub fn export_to_json(pdfs: &[PdfListResult], export_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn export_to_csv(pdfs: &[PdfListResult], export_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn export_to_markdown(pdfs: &[PdfListResult], export_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        md_data.push_str(&format!("- **Export Date**: {}\n\n", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S")));
    ...
    pub fn export_to_yaml(pdfs: &[PdfListResult], export_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn export_to_html(pdfs: &[PdfListResult], export_path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for pdf in pdfs {
            html_data.push_str(&format!("<tr><td>{}</td><td>{}</td><td>{}</td></tr>\n",
                pdf.filename,
                pdf.size_human,
                pdf.modified.format("%Y-%m-%d %H:%M:%S")
            ));
